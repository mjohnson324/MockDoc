...
# For Setup from scratch uncomment all code.
...
# New Additions
current_last_doc = Doctor.last
last_id = current_last_doc.id
last_street_assigned = current_last_doc.address
# New Doctors
...
new_docs = Doctor.where("id > #{last_id}")
new_doc_count = new_docs.length
puts('Doctors done')

# Addresses
last_street_assigned.split(" ")[2][0..2].to_i
street_number = last_street_assigned + 1
addresses = []
until addresses.length >= new_doc_count
...
# Join Specialties
DoctorSpecialty.create!(specialty_id: specialties[0].id, doctor_id: _.id)
